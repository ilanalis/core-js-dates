...
function getDayName(date) {
  ...
  return dayNames[utcTime.getDay()];
}
/**
...
function getNextFriday(date) {
  ...
  const additionalWeek = date.getDay() === 5 ? 7 : 0;
  nextFriday.setDate(date.getDate() + daysUntilFriday + additionalWeek);
  return nextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const totalDiff = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  return Math.ceil(totalDiff / (1000 * 60 * 60 * 24)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  if (
    checkDate >= new Date(period.start) &&
    checkDate <= new Date(period.end)
  ) {
    return true;
  }
  return false;
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${formattedHours}:${minutes}:${seconds} ${time}`;
}
/**
...
function getWeekNumberByDate(date) {
  ...
  const closestMonday = new Date(Date.UTC(localDate.getUTCFullYear(), 0, 1));
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startDayArray = period.start.split('-');
  const endDayArray = period.end.split('-');
  const endDate = new Date(
    Date.UTC(endDayArray[2], endDayArray[1] - 1, endDayArray[0])
  );
  const currentDate = new Date(
    Date.UTC(startDayArray[2], startDayArray[1] - 1, startDayArray[0])
  );
  const millisecondsPerDay = 24 * 60 * 60 * 1000;
  const daysDifference = Math.ceil(
  ...
  const resultArray = [];
  let currentWorkingDayNumber = 1;
  let currentOffDayNumber = 0;

  for (let i = 0; i <= daysDifference; i += 1) {
    if (currentWorkingDayNumber > 0) {
      if (currentWorkingDayNumber <= countWorkDays) {
        currentWorkingDayNumber += 1;
        resultArray.push(new Date(currentDate));
      } else {
        currentWorkingDayNumber = 0;
        currentOffDayNumber = 1;
        i -= 1;
        currentDate.setDate(currentDate.getDate() - 1);
      }
    } else if (currentOffDayNumber > 0) {
      if (currentOffDayNumber <= countOffDays) {
        currentOffDayNumber += 1;
      } else {
        currentOffDayNumber = 0;
        currentWorkingDayNumber = 1;
        i -= 1;
        currentDate.setDate(currentDate.getDate() - 1);
      }
